...
sExtractorParamFile1 = 'maskMaker/default.sex'
...
def boxCountingDistanceK(k, data1, data2, metrica):
    ...
    print 'exiting boxCountingDistanceK'
...
def optimizeGa(gaTol,gaATol,dataFile1,dataFile2, nprocess=2):
    if(nprocess<2):
        raise Exception("You must specify nprocess>1 (at least one headnode, and a worker)")
 
    # iterate over different values of metric parameters
    metricsGa = []
    k_values = [0.1, 0.4, 0.6, 0.8, 1.0, 1.4, 1.6, 1.8, 2.0, 2.5, 3.0, 3.5, 4.0, 5.0]
    output_file = "optimize/ga.csv"
    for gaMTol in gaTol:
    	for gaAngTol in gaATol:
	        # edit DETECT_THRESH for sExtractor
	        for k in k_values:
	            for line in fileinput.FileInput(sExtractorParamFile1, inplace=1):
	                if not desiredParam in line:
	            ...
	            print "Running boxCounting metric"
	            try:
	                nm = runMetricK(k, "output/r1.csv","output/r2.csv","sGa")
	                nm.insert(0,gaMTol)
	                metricsGa.append(nm)
    ...
    for cv in sm:
        ...
        for k in k_values:
            ...
            try:
                ...
                print(df)
                df.to_csv(output_file, index=False)
                process = os.popen("mkdir output/S"+str(cv)+"_k"+str(k))
                process.read()
                process = os.popen("mv output/r1.csv output/r2.csv output/S"+str(cv)+"_k"+str(k)+"/")
                process.read()
            except:
...
if __name__ == "__main__":
    ...
    # optimizeGa(gaTol=[0.0,0.02,0.04,0.06,0.08,0.1,0.15,0.2],gaAngTol=0.02,dataFile1="test1000/1000_S_k20.csv",dataFile2="test1000/1000_E_k20.csv",nprocess=n)
    optimizeGa(gaTol=[0.03,0.04,0.05],gaATol=[0.01,0.02,0.04],dataFile1="test1000/1000_S_k20.csv",dataFile2="test1000/1000_E_k20.csv",nprocess=n)
